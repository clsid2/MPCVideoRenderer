...
HRESULT CDX11VideoProcessor::InitializeD3D11VP(const FmtConvParams_t& params, const UINT width, const UINT height, const CMediaType* pmt)
{
	...
	if (m_bVPScaling) {
		if (m_bHdrPassthroughSupport && m_bHdrPassthrough && SourceIsHDR()) {
			if (CheckDriverNVIDIA(572) && m_InternalTexFmt != DXGI_FORMAT_R16G16B16A16_FLOAT) {
				superRes = m_iVPSuperRes;
			}
		} else if ((m_InternalTexFmt == DXGI_FORMAT_B8G8R8A8_UNORM || m_bVPUseRTXVideoHDR) && (params.CDepth == 8 || !m_bACMEnabled)) {
			superRes = m_iVPSuperRes;
		}
	}
...
#if USEPRESCALESHADERS
void CDX11VideoProcessor::UpdatePreScaleTextures()
{
	m_TexsPreScale.Release();
	...
	DLogIf(FAILED(hr), L"CDX11VideoProcessor::UpdatePreScaleTextures() : m_TexsPreScale.CheckCreate() failed with error {}", HR2Str(hr));
}
#endif
...
HRESULT CDX11VideoProcessor::Process(ID3D11Texture2D* pRenderTarget, const CRect& srcRect, const CRect& dstRect, const bool second)
{
	...
	if (m_D3D11VP.IsReady()) {
		if (!(m_iSwapEffect == SWAPEFFECT_Discard && (m_VendorId == PCIV_AMDATI || m_VendorId == PCIV_INTEL))) {
			...
			if (!bNeedShaderTransform && !numSteps && !numPrescaleSteps) {
...
#if USEPRESCALESHADERS
	if (m_pPreScaleShaders.size()) {
		str.append(L"\n\nPre scale pixel shaders:");
		for (const auto& pshader : m_pPreScaleShaders) {
			str += std::format(L"\n  {}", pshader.name);
		}
	}
#endif
...
void CDX11VideoProcessor::Configure(const Settings_t& config)
{
	...
	if (changeSuperRes) {
		...
		if (m_bVPScaling) {
			...
			} else if (m_srcParams.CDepth == 8 || !m_bACMEnabled) {
				superRes = m_iVPSuperRes;
			}
		}
		m_bVPUseSuperRes = (m_D3D11VP.SetSuperRes(superRes) == S_OK);
...
HRESULT CDX11VideoProcessor::DrawStats(ID3D11Texture2D* pRenderTarget)
{
	...
	if (m_strCorrection || m_pPreScaleShaders.size() || m_pPostScaleShaders.size() || m_bDitherUsed) {
		str.append(L"\nPostProcessing:");
		if (m_strCorrection) {
...
#if USEPRESCALESHADERS
		if (m_pPreScaleShaders.size()) {
			str += std::format(L" pre-shaders[{}],", m_pPreScaleShaders.size());
		}
#endif
		if (m_pPostScaleShaders.size()) {
